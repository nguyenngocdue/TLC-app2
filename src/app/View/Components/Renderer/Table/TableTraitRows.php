...
trait TableTraitRows
{
    ...
    private function makeTd($columns, $dataLineObj, $no, $dataLineIndex, $batchLength, $tableDebug)
    {
        ...
        foreach ($columns as $index => $column) {
            ...
            $td = "<td class='$fixedLeft $fixedRight bg-white dark:border-gray-600 border-b  $tinyText $breakWords $cellClassList $hidden $borderRight $borderGray $align'";
            $td .= $styleStr;
        ...
        foreach ($dataSource as $dataLineIndex => $dataLine) {
            ...
            if ($this->groupBy) {
                ...
                if ($index !== $lastIndex) {
                    ...
                    $tr .= "<td class='$fixedLeft bg-white p-2 border-b text-lg font-bold text-gray-600 dark:text-gray-300' colspan=$colspan>{$index}</td>";
                    $tr .= "</tr>";
                    $trs[] = $tr;
                }
            }
