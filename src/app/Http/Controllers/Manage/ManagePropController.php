...
abstract class ManagePropController extends Controller
{
    ...
    {   
        $type = $this->type;
        $patch = storage_path() . "/json/$type/manage.json";
        if( !file_exists($patch)){
            ...
            if(empty($diff1) && empty($diff2)){
                ...
                foreach($diff2 as $value){
                    ...
                    $type = Schema::getColumnType($type.'s', $value);
    ...
    public function store(Request $request){
        ...
        try {
            Storage::disk('json')->put("{$this->type}/manage.json",$jsonManageUser,'public');
    ...
    public function destroy($name){
        $patch = storage_path() . "/json/{$this->type}/manage.json";
        $dataManageUser = json_decode(file_get_contents($patch), true);
        unset($dataManageUser[$name]);
        try {
            Storage::disk('json')->put("{$this->type}/manage.json",json_encode($dataManageUser),'public');
            Toastr::success('Save file json successfully', 'Save file json');
