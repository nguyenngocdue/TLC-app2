	...
	private function getDataHumanCapitalNewEmployees($params)
	{
		...
		foreach ($years as $year) {
			$strSQL1 .= "	SUM((tb.c1_new_employees_by_{$year} + tb.c2_new_employees_by_{$year}) - (tb.c3_new_employees_by_{$year} + tb.c4_new_employees_by_{$year})) AS total_new_employees_by_year_{$year},
							...
							SUM((tb.c1_new_employees_female_by_year_{$year} + tb.c2_new_employees_female_by_year_{$year}) - (tb.c3_new_employees_female_by_year_{$year} + tb.c4_new_employees_female_by_year_{$year})) AS new_employees_female_by_half_year_{$year},
							...
							SUM((tb.c1_new_employees_male_by_year_{$year} + tb.c2_new_employees_male_by_year_{$year}) - (tb.c3_new_employees_male_by_year_{$year} + tb.c4_new_employees_male_by_year_{$year})) AS new_employees_male_by_half_year_{$year},
			...
			$strSQL2 .= " 
							#New Employees for the full year 
								...
								CASE WHEN DATE_FORMAT(us.first_date, '%Y-%m-%d') <= '{($year - 1)}-12-31' 
										AND DATE_FORMAT(us.last_date, '%Y-%m-%d') <= '{($year - 1)}-12-31'
										AND DATE_FORMAT(us.last_date, '%Y-%m') = '{($year - 1)}-12' 
							...
							#New Employees for the first half year 
								...
								CASE WHEN DATE_FORMAT(us.first_date, '%Y-%m-%d') <= '{($year - 1)}-06-30' 
										AND DATE_FORMAT(us.last_date, '%Y-%m-%d') <= '{($year - 1)}-06-30'
										AND DATE_FORMAT(us.last_date, '%Y-%m') = '{($year - 1)}-06' 
