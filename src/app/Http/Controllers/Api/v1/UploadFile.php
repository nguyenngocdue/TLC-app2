...
class UploadFile extends Controller
{
    ...
    private function setAttachmentParent($modelName, $modelId, $colNamesHaveAttachment)
    {
        if (!$modelName || !$modelId) {
            return response()->json('Please input' . !$modelName ? 'model' : 'model_id');
        }
        $model = $modelName::find($modelId);
        $ownerId =  (int)Auth::user()->id;
        $uploadedIdColumnNames = DB::table('attachments')
            ->where('owner_id', $ownerId)
            ->where('object_id', null)
            ->where('object_type', null)
            ->select('id', 'category')
            ->get()->toArray();
        $arrayIdAttachmentAndIdCategory =  array_column($uploadedIdColumnNames, 'category', 'id');
        $idAndNameFields = DB::table('fields')->select('id', 'name')->get()->toArray();
        $arrayIdAndNameField = array_column($idAndNameFields, 'name', 'id');
    }
}
