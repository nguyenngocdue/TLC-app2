...
const smartFilter4 = (dataSource, column, operator, value) => {
    return dataSource.filter((row) => {
        ...
        switch (operator || '=') {
            case '=':
                if (Array.isArray(row[column])) result = dumbIncludes4(value, row[column])
                else result = row[column] == value
                return result
            case '!=':
                if (Array.isArray(row[column])) result = !dumbIncludes4(value, row[column])
                else result = row[column] != value
                            ...
                            if (hit[item_att] == undefined) {
                                ...
                                if (debugListener)
                                    console.log(
                                        'Requested column',
                                        item_att,
                                        'not found in response - assigning default value',
                                        defaultValue,
                                    )
                            } else {
                                value = hit[item_att]
