<x-renderer.card style=" border-gray-300 rounded-lg">
    ...
    {{-- @dump($value) --}}
    <div class="flex my-2">
        <x-renderer.comment labelName="{{$labelName}}" btnUpload="{{$value['btnUpload'] ?? false}}" name="{{$name}}" type="{{$type}}" id="{{$id}}" :dataComment="$value" action={{$action}} readonly="{{true}}"></x-renderer.comment>
        @if(!isset($value['btnUpload']))
