<div class="input-group">
    ...
    <input type="text" autocomplete="off" class="form-control float-right bg-white  border border-gray-300 text-gray-900 rounded-md p-2.5 dark:placeholder-gray-400 block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:outline-none focus:shadow-outline-purple focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" id="{{$name}}" name="{{$name}}" value="{{$value}}">
  </div>
<script>
    ...
    $('[id="'+"{{$name}}"+'"]').on('apply.daterangepicker', function(ev, picker) {
      $(this).val('Q'+picker.startDate.format('Q/YYYY') + ' - ' + 'Q'+picker.endDate.format('Q/YYYY'));
    });
