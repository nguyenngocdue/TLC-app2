<div class="no-print flex h-full bg-gray-50 dark:bg-gray-900 overflow-hidden" :class="{ 'overflow-hidden': isSideMenuOpen }">
    ...
    <aside class="z-20 hidden w-80 overflow-y-auto bg-white dark:bg-gray-800 md:block flex-shrink-0">
        <div class="py-4 text-gray-500 dark:text-gray-300">
            <x-homepage.logo />
            <x-homepage.main-menu :items="$items" />
    ...
    <aside class="fixed inset-y-0 z-20 flex-shrink-0 w-80 mt-16 overflow-y-auto bg-white dark:bg-gray-800 md:hidden" x-show="isSideMenuOpen" x-transition:enter="transition ease-in-out duration-150" x-transition:enter-start="opacity-0 transform -translate-x-20" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in-out duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0 transform -translate-x-20" @click.away="closeSideMenu" @keydown.escape="closeSideMenu">
        <div class="py-4 text-gray-500 dark:text-gray-300">
            <x-homepage.logo />
            <x-homepage.main-menu :items="$items" />
            {{-- <x-homepage.actionbutton /> --}}
