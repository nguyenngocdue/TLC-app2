<form action="{{$route}}" method="post" class="w-full lg:w-1/3 p-2 lg:p-0">
    ...
    <div class="flex items-center lg:justify-end">
        <div class="mr-1 w-24">
            ...
            <select name="page_limit" class="block w-full rounded-md border bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 px-1 py-2 text-right placeholder-slate-400 shadow-sm focus:border-purple-400 dark:focus:border-blue-600 focus:outline-none sm:text-sm">
                @foreach([10,20,30,40,50,100] as $perPage)
                <option value="{{$perPage}}" @selected($pageLimit==$perPage)>{{$perPage}} / page</option>
                @endforeach
            </select>
        </div>
