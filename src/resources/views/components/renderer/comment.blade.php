...
$authorName = $user->name;// $userDB->pluck('name')->first();
$position = $user->position_rendered;// $userDB->pluck('position_rendered')->first();
...
<x-renderer.card style="bg-gray-250  py-3 border-gray-300 rounded-lg">
    <div class="grid grid-cols-12 gap-2 flex-nowrap ">
        <div class="col-span-4">
            <div class="border bg-gray-300  rounded-lg w-full border-gray-300 p-0.5 ">
                <x-renderer.avatar-name title="{{$authorName}}" description="{{$position}}" href="http://www.google.com">?</x-renderer.avatar-name>
                <input name="owner_id" value="{{Auth::user()->id}}" class='hidden' type='text'>
            </div>
        </div>

        <div class="col-span-4">
            <input value="{{$position}}" readonly class='bg-gray-300  border border-gray-300 text-gray-900  rounded-lg  p-2.5   dark:placeholder-gray-400  block w-full text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none  focus:shadow-outline-purple focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input' type='text'>
        </div>
    ...
    <div class="pt-2">
        ...
        {{-- <textarea name="{{$name}}" class='hidden'>{{old($name, $content) }}</textarea> --}}
    </div>
</x-renderer.card>
