...
<div id="cluster_parent_{{$cluster->id}}" data-id="cluster_{{$cluster->id}}">
    ...
    <input id="txt_cluster_{{$cluster->id}}" value="{{$cluster->name}}" class="{{$classCluster}} {{$hidden}}" onblur="onClickToCommit({{$cluster->id}},'lbl_cluster','txt_cluster','caption_cluster', route_cluster)">
    <div id="cluster_{{$cluster->id}}" data-id="cluster_{{$cluster->id}}" class="flex bg-gray-50 m-2 border rounded p-2">
        ...
        <div class="m-1 bg-gray-200 p-2 rounded h-9">
            ...
            <button class="{{$classButton}} px-4 {{$groupWidth}}" type="button" onclick="addANew({{$cluster->id}}, route_group)">+ Add a List</button>
        </div>
    </div>
</div>
@endforeach