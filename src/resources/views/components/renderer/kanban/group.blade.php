...
<div id="group_parent_{{$group->id}}" data-id="group_{{$group->id}}" class="m-1 bg-gray-200 p-2 rounded">
    ...
    <input id="txt_group_{{$group->id}}" value="{{$group->name}}" class="{{$classGroup}} {{$hidden}}" onblur="onClickToCommit({{$group->id}},'lbl_group','txt_group','caption_group', route_group)">
    <div id="group_{{$group->id}}" data-id="group_{{$group->id}}" class="grid gap-1 {{$groupWidth}}">
       <x-renderer.kanban.task
    ...
    <button class="{{$classButton}} mt-2 {{$groupWidth}}" type="button" onclick="addANew({{$group->id}}, route_task)">+ Add a Card</button>
