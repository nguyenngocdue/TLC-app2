...
<form action="{{ $route }}" id="{{ $entity }}" method="GET">
                ...
                @foreach ($columns as $key => $value)
                    @php
                        $title = isset($value['title'])
                            ? $value['title']
                            : ucwords(str_replace('_', ' ', $value['dataIndex']));
                        ...
                        $showNow = isset($value['showNow']) ? true : '' ?? '';
                    @endphp
                    <div id="name_{{ $name }}" class="col-span-2 {{ $name }} {{ $firstHidden }}">
                        @switch($renderer)
                                    ...
                                    @foreach ($errors->getMessages() as $field => $message)
                                        @if ($field === $name && $field !== 'end_date' && $field !== 'end_date')
                                            <span class="text-xs" role="alert">
                                                <p class="mt-1.5 text-red-600 font-semibold">(Please select the correct
                                                    <strong>{{ ucwords(str_replace('_', ' ', $field)) }}</strong> format)
                                                </p>
                                            </span>
                                        @endif
                                    @endforeach
                            ...
                            @case('picker_date')
                                <x-reports.picker-date1 title="{{ $title }}" name="{{ $name }}"
                                    allowClear={{ $allowClear }} value="{{ $date }}"
    ...
    <div class="py-2">
        <x-renderer.button htmlType="submit" type="primary"><i class="fa-sharp fa-solid fa-check"></i> Apply
            Filter</x-renderer.button>
        <x-renderer.button htmlType="submit" click="resetFilter()" type="secondary"><i
