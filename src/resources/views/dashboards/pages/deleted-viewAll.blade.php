...
<main class="h-full overflow-y-auto">
    <div class="container mx-auto grid px-6">
        ...
        <div class="mt-2 grid grid-cols-2 gap-5">
            ...
            <form action="{{ route($type . '.update', Auth::id()) }}" method="post">
                @method('PUT')
                @csrf
                <div class="mt-2 flex">
                    <div class="mr-1 w-12">
                    ...
                    <tbody class="divide-y bg-white dark:divide-gray-700 dark:bg-gray-800">
                        ...
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3 text-sm">
                                <button class="focus:shadow-outline-gray btn-delete-user rounded-lg px-2 py-2 text-sm font-medium leading-5 text-red-600 focus:outline-none dark:text-red-400" data-url="{{ route($type . '.destroy', $user->id) }}" ​ type="button"><i class="fas fa-trash"></i></button>
                            </td>
                            ...
                            <td class="{{ $key1 . '_td' }} px-4 py-3 text-sm">
                                <a href="{{ route($type . '.edit', $user[$value['column_name']]) }}" class="text-sm font-normal text-blue-500">{{ $result }}</a>
                            </td>
                            @else
                            <td class="{{ $key1 . '_td' }} px-4 py-3 text-sm">

                                <a href="{{ route($type . '.edit', $user[$value['column_name']]) }}">{{ $user[$value['column_name']] }}</a>
                            </td>
                            @endif
                            @else
                            <td class="{{ $key1 . '_td' }} px-4 py-3 text-sm">
                                ...
                                <div title="{{ $user[$value['column_name']] }}">
                                    ...
                                    <x-renderer.tag>
                                        ABC
                                        @php
                                        echo (!is_array($user->{$item['relationship']}) ? $user->{$item['relationship']}->count() : '0')." items";
                                        @endphp
                                    </x-renderer.tag>
                                    @else
                                    Render Failed
                            ...
                            <td>
                                @isset($user->{$item['relationship']})
                                <x-renderer.tag value="{{ !is_array($user->{$item['relationship']}) ? $user->{$item['relationship']}->count() : '0' }} items">888</x-renderer.tag>
                                @else
                                <p class="rounded-md bg-red-300 px-2 py-0 text-xs font-semibold leading-tight text-red-400 dark:bg-red-500 dark:text-green-100">
                                    Render Failed
                                </p>
