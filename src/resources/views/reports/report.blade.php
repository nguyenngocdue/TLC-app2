        ...
        <div class="grid grid-cols-12 gap-1">
            <div class="col-span-2">
                <div class="no-print justify-end pl-4 pt-5">
                    <div
                        class="w-full no-print rounded-lg border bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-600 mb-5 p-3">
                        <x-reports.dropdown8 title="Mode" name="forward_to_mode" routeName="{{ $routeName }}"
                            :allowClear="false" :dataSource="$modeParamDataSource" typeReport='{{ $typeReport }}' entity='{{ $entity }}'
                            modeOption='{{ $mode }}' :itemsSelected="$params" />
                    </div>
                </div>
            </div>
    ...
    <div class="px-4">
        ...
        @if ($typeOfView === 'report-pivot')
            <x-renderer.report.pivot-table showNo={{ true }} modeType="{{ $modeType }}" :tableDataHeader="$tableDataHeader"
                ...
                page-limit="{{ $pageLimit }}" headerTop=10 maxH="{{ $maxH }}" showPaginationTop="true"
        ...
        @else
            <x-renderer.table showNo={{ true }} :dataHeader="$tableDataHeader" :columns="$tableColumns" :dataSource="$tableDataSource"
                rotate45Width={{ $rotate45Width }} maxH="{{ $maxH }}"
                tableTrueWidth={{ $tableTrueWidth ? 1 : 0 }} page-limit="{{ $pageLimit }}" showPaginationTop="true"
                topLeftControl="{!! $tl !!}" topCenterControl="{!! $tc !!}"
                topRightControl="{!! $tr !!}" />
        @endif
    </div>
