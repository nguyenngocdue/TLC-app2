...
<div class="px-4 ">
    ...
    <x-renderer.table topCenterControl="{!!$tc!!}" :columns="$tableColumns" :dataSource="$tableDataSource" showNo={{true}} maxH="{{$maxH}}" rotate45Width={{$rotate45Width}} groupBy="{{$groupBy}}" groupByLength="{{$groupByLength}}" tableTrueWidth_disabled={{$tableTrueWidth?1:0}} />
</div>
@endsection
