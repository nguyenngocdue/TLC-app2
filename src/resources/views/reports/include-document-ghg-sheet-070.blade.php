...
@php
	...
	$isEmptyData = count($dataRender) ? false : true;
@endphp
{{-- @dump($columnType) --}}
...
<div class="rounded-lg border-gray-950 border-2 overflow-hidden overflow-x-auto max-w-[1800px]  scrollbar-thumb-gray-100 ">
	...
	<table class="tg whitespace-no-wrap w-full text-sm overflow-hidden border-gray-900">
		<thead>
		<tr class="rounded-t-lg bg-gray-100">
		...
		<tbody>
			@if(!$isEmptyData)
														...
														@case('years')
																@foreach($years as $key => $year)
																	@php
																		...
																		//dd($dataIndex);
																	@endphp
																		@include('components.reports.tco2e', ['widthCell'=> $widthCell, 'class1' => '', 'tco2e' => $valueForColType, 'difference' => $numberComparison])
																@endforeach
															@break
														@case ('months' || 'quarters')
																@php
				...
				<tr class="pb-4 border-b border-gray-600">
					@php
				...
				@else
				<td colspan="14"><div class="p-10 text-center text-gray-500 ">
					<i class="fa-regular fa-box-open text1-4xl"></i>
					There is no item to be found
			@endif
		</tbody>
	</table>
</div>